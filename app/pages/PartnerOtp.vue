...
import { getApiErrorMessage, type AccountMessageResponse, type AuthVerifyEmailResponse } from '~/types/auth'
...
useHead({
  title: 'Verify Partner OTP - Bread + Butter'
})

const toast = useToast()
const route = useRoute()
const { executeAction, apiRequest } = useApiMode()
const { verifyEmail } = useAuth('partner')
  ...
  try {
    await executeAction({
      uiOnly: async () => {
        toast.add({
          title: 'OTP verified',
          description: 'Your partner account is ready.',
          color: 'success'
        })
        await navigateTo('/partners')
      },
      api: () => verifyEmail(otpId.value, event.data.pinCode),
      onApiSuccess: async (res: AuthVerifyEmailResponse) => {
        toast.add({
          title: 'OTP verified',
  ...
  try {
    await executeAction({
      uiOnly: async () => {
        toast.add({
          title: 'OTP resent',
          description: 'A new 4-digit code was sent to your email.',
          color: 'info'
        })
      },
      api: () => apiRequest(`/partner/otp/resend-email/${otpId.value}`, {
        method: 'PATCH',
        authenticated: false
      }),
      onApiSuccess: async (res: AccountMessageResponse) => {
        toast.add({
          title: 'OTP resent',
...
<template>
  <div class="flex items-center justify-center p-4 bpb-pattern h-screen">
    <UPageCard class="bread-container w-full max-w-md ring ring-transparent p-4 sm:p-6 bg-toast-50">
      <div class="space-y-6 text-center">
        ...
        <div class="space-y-1">
          <h1 class="text-2xl font-serif font-bold text-toast-700">Verify Partner Email</h1>
        ...
        <UForm :schema="otpSchema" :state="state" class="space-y-6" @submit="submitOtp">
          ...
          <div class="space-y-3 pt-2">
            ...
            <div class="flex justify-center items-center gap-1 text-xs text-toast-800">
              ...
              <button
                type="button"
                class="text-toast-600 hover:text-toast-800 font-bold underline cursor-pointer"
          ...
          <div class="flex justify-between items-center text-xs border-t border-toast-600/15 pt-4">
            <ULink to="/partners/login" class="text-toast-600 hover:text-toast-800 font-medium">
              Back to Sign In
            </ULink>
            <ULink to="/partners/forgot-password" class="text-toast-600 hover:text-toast-800 font-medium">
              Forgot Password?
            </ULink>
          </div>
        </UForm>
