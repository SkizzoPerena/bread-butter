...
import { useSubEventGuestsManager } from '~/composables/useSubEventGuestsManager'
  ...
  <UContainer class="space-y-6 py-8 pb-12">
    ...
    <template v-else-if="subEventRecord">
      ...
      <UPageCard class="white-bread-container space-y-4">
        <div>
          <p class="text-lg font-semibold text-highlighted">Edit Sub-Event</p>
          <p class="text-sm text-muted">
            Update details for this schedule item. Date must be before {{ parentEventDateLabel }}.
          </p>
        </div>

        <form class="space-y-4" @submit.prevent="handleSaveChanges">
      ...
      <UPageCard class="white-bread-container space-y-4">
        <div class="flex flex-wrap items-start justify-between gap-3">
          ...
          <UButton
            ...
            :disabled="!canInviteAll || isInvitingAll"
        ...
        >
          ...
          <p class="mt-1 max-w-sm text-sm text-muted">
            Add guests from the Guest List page before sending sub-event invitations.
          </p>
        </div>
