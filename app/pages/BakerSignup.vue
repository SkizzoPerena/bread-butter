...
definePageMeta({
  layout: false,
  alias: ['/bakers/signup', '/baker/signup', '/bakers-signup', '/baker-signup', '/BakersSignup']
})

useHead({
  title: 'Baker Registration - Bread + Butter',
  meta: [
    {
      name: 'description',
      content: 'Create your Bread + Butter Baker account.'
    }
  ]
})
...
const state = useState<Schema>('baker-signup-draft', () => {
  if (import.meta.client) {
    try {
      const saved = sessionStorage.getItem('bpb_baker_signup_draft')
...
watch(state, (val) => {
  if (import.meta.client) {
    try {
      sessionStorage.setItem('bpb_baker_signup_draft', JSON.stringify(val))
    } catch {
...
async function onSubmit(payload: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    if (import.meta.client) {
      sessionStorage.removeItem('bpb_baker_signup_draft')
    }
    ...
    toast.add({
      ...
      description: getApiErrorMessage(error, 'Unable to create your baker account.'),
      color: 'error'
    })
  } finally {
        ...
        <UForm :schema="schema" :state="state" class="space-y-5" @submit="onSubmit">
          <div class="flex justify-between mt-1 mb-6 items-center">
            <div class="text-left text-sm text-bread-400">
              <div class="text-xl font-serif font-semibold text-white">Become a Baker</div>
              ...
              <UCheckbox v-model="state.tnc" name="tnc">
                <template #label>
                  <span class="text-sm text-white">
                    ...
                    <ULink :to="{ path: '/terms', query: { from: 'baker-signup' } }" class="text-bread-400 font-medium">
          ...
          <UButton type="submit" block :loading="isSubmitting" class="!bg-toast-500 hover:!bg-toast-600 !text-white font-semibold cursor-pointer">Sign up</UButton>
          <div class="text-sm text-center mt-1">
            Already a baker? <ULink to="/bakers/login" class="text-bread-400 font-medium">Sign in</ULink> instead.
          </div>
        </UForm>
      </div>
    </UPageCard>
